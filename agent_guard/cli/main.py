"""Agent Guard CLI — Enterprise AI Engineering Control Plane."""
...
app = typer.Typer(
    name="ag",
    help="Agent Guard — Secure, token-optimized AI engineering control plane.",
...
def init(
    ...
    """Initialize Agent Guard for a project."""
    from agent_guard.models import AgentPlatform, OnboardingMode
    from agent_guard.onboarding.engine import detect_and_build_config, save_config
    ...
    console.print(Panel(
        f"[bold green]Agent Guard initialized for {config.project_name}[/]\n"
...
def optimize(path: str = typer.Argument(None, help="Project root (default: cwd)")):
    ...
    from agent_guard.detection.engine import StackDetector
    from agent_guard.governance.policies import get_policies_for_stack
    from agent_guard.optimizer.engine import TokenOptimizer
...
def audit(path: str = typer.Argument(None, help="Project root (default: cwd)"), count: int = typer.Option(20, "--count", "-n")):
    ...
    from agent_guard.telemetry.audit import AuditLog

    root = _resolve_root(path)
    log = AuditLog(root / ".agent-guard" / "audit")
...
def doctor(path: str = typer.Argument(None, help="Project root (default: cwd)")):
    """Diagnose Agent Guard setup health."""
    from agent_guard.onboarding.engine import load_config, CONFIG_FILE
    ...
    table = Table(title="Agent Guard Health Check", show_header=True)
...
def explain(rule_id: str = typer.Argument(..., help="Policy rule ID (e.g. DB-001)")):
    ...
    from agent_guard.governance.policies import ALL_POLICIES
...
def simulate(command: str = typer.Argument(..., help="Command to simulate")):
    ...
    from agent_guard.models import ExecutionRequest
...
def validate(path: str = typer.Argument(None, help="Project root (default: cwd)")):
    ...
    from agent_guard.detection.engine import StackDetector
    from agent_guard.governance.engine import GovernanceEngine
    from agent_guard.compliance.engine import ComplianceEngine
    from agent_guard.optimizer.engine import TokenOptimizer
    from agent_guard.governance.policies import get_policies_for_stack
...
def benchmark(
    path: str = typer.Argument(None, help="Project root (default: cwd)"),
    output: str = typer.Option(".agent-guard/reports", "--output", "-o", help="Output directory for reports"),
):
...
@app.command()
def version():
    """Show Agent Guard version."""
    console.print(f"Agent Guard v{__version__}")
